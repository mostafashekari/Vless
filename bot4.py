...
# ✨ تابع جدید برای خواندن UUID از فایل کانفیگ
...
# ✨ تابع جدید برای نمایش کانفیگ(ها)
async def cb_myconfig(update: Update, ctx: CallbackContext):
    ...
    for line in lines:
        ...
        # اگر کاربر ادمین است، همه کانفیگ‌ها را جمع‌آوری کن
        if user_id == ADMIN_ID:
            user_configs.append({'name': name, 'port': port})
        # اگر کاربر عادی است، فقط کانفیگ خودش را پیدا کن
        elif int(line_user_id) == user_id:
            user_configs.append({'name': name, 'port': port})
            break # کاربر عادی فقط یک کانفیگ دارد، پس حلقه را متوقف کن

    if not user_configs:
        await update.callback_query.message.reply_text("شما هنوز کانفیگی نساخته‌اید. روی دکمه 'ساخت کانفیگ جدید' بزنید.")
        return

    # حالا لینک‌ها را بساز و نمایش بده
    response_text = []
...
def main():
    ...
    # ✨ ثبت کردن هندلر جدید برای دکمه "کانفیگ من"
